...
import { MapPin, X, Clock, Calendar, ChevronRight } from "lucide-react";
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (configsJson.status === 'success') setSiteConfigs(configsJson.data);
            } catch (error) {
        ...
        <main className="min-h-screen bg-carapita-green flex flex-col font-sans selection:bg-carapita-gold selection:text-white">
            ...
            <section className="py-20 px-6 md:px-12 max-w-7xl mx-auto w-full">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-20">
                    ...
                    <div className="bg-white/5 border border-white/10 p-8 rounded-3xl backdrop-blur-sm hover:border-carapita-gold/50 transition-all group">
                        ...
                        <h3 className="text-xl font-serif text-white mb-4">{t('passeios_express_title')}</h3>
                        <p className="text-white/50 text-sm leading-relaxed mb-6">{t('passeios_express_desc')}</p>
                        <ul className="space-y-3 text-xs text-white/70 uppercase tracking-widest">
                            ...
                            <li className="flex items-center gap-2"><ChevronRight size={12} className="text-carapita-gold" /> {lang === 'EN' ? 'Fátima Sanctuary' : 'Santuário de Fátima'}</li>
                            <li className="flex items-center gap-2"><ChevronRight size={12} className="text-carapita-gold" /> {lang === 'EN' ? 'Medieval Village' : 'Vila Medieval'}</li>
                        </ul>
                    </div>
                    ...
                    <div className="bg-white/5 border border-white/10 p-8 rounded-3xl backdrop-blur-sm hover:border-carapita-gold/50 transition-all group">
                        ...
                        <h3 className="text-xl font-serif text-white mb-4">{t('passeios_templaria_title')}</h3>
                        <p className="text-white/50 text-sm leading-relaxed mb-6">{t('passeios_templaria_desc')}</p>
                        <ul className="space-y-3 text-xs text-white/70 uppercase tracking-widest">
                            <li className="flex items-center gap-2"><ChevronRight size={12} className="text-carapita-gold" /> {lang === 'EN' ? 'Convent of Christ' : 'Convento de Cristo'}</li>
                            <li className="flex items-center gap-2"><ChevronRight size={12} className="text-carapita-gold" /> {lang === 'EN' ? 'Tomar Castle' : 'Castelo de Tomar'}</li>
                            <li className="flex items-center gap-2"><ChevronRight size={12} className="text-carapita-gold" /> {lang === 'EN' ? 'Pegões Aqueduct' : 'Aqueduto dos Pegões'}</li>
                        </ul>
                    </div>
                    ...
                    <div className="bg-white/5 border border-white/10 p-8 rounded-3xl backdrop-blur-sm hover:border-carapita-gold/50 transition-all group">
                        ...
                        <h3 className="text-xl font-serif text-white mb-4">{t('passeios_natureza_title')}</h3>
                        <p className="text-white/50 text-sm leading-relaxed mb-6">{t('passeios_natureza_desc')}</p>
                        <ul className="space-y-3 text-xs text-white/70 uppercase tracking-widest">
                            <li className="flex items-center gap-2"><ChevronRight size={12} className="text-carapita-gold" /> {lang === 'EN' ? 'Mira de Aire Caves' : 'Grutas de Mira de Aire'}</li>
                            <li className="flex items-center gap-2"><ChevronRight size={12} className="text-carapita-gold" /> {lang === 'EN' ? 'Dinosaur Footprints' : 'Pegadas Dinossauros'}</li>
                ...
                <div className="text-center mb-16">
                    <h2 className="text-3xl font-serif text-white mb-4">{t('passeios_todos_pontos')}</h2>
                    <div className="w-20 h-1 bg-carapita-gold mx-auto opacity-50 rounded-full" />
                </div>

                {loading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-pulse">
                        {[1, 2, 3, 4, 5, 6].map(i => (
                            <div key={i} className="bg-white/5 h-96 rounded-3xl" />
                        ))}
                    </div>
                ) : (
                        ...
                        {passeios.map((passeio, idx) => (
                            <div key={idx}
                                ...
                                <div className="h-72 overflow-hidden relative">
                                    <img src={passeio.img} alt={passeio.nome} className="w-full h-full object-cover transform duration-1000 group-hover:scale-110" />
                                    ...
                                    <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-carapita-green to-transparent opacity-80" />
                                </div>
            ...
            <section className="py-32 px-6 text-center bg-carapita-dark/30 border-t border-white/5 relative overflow-hidden">
                ...
                <div className="max-w-2xl mx-auto relative z-10">
                    <h2 className="text-4xl font-serif text-white mb-8">{t('passeios_cta_title')}</h2>
                    <p className="text-white/60 mb-12">{t('passeios_cta_desc')}</p>
                    <button
                        onClick={goToBooking}
                        className="bg-carapita-gold text-carapita-dark px-12 py-4 rounded-full text-xs font-bold uppercase tracking-mega hover:bg-white transition-all transform hover:scale-105 cursor-pointer">
                        {t('btn_reservar_now')}
                    </button>
                </div>
            </section>
            ...
            {passeioSelecionado && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-black/90 backdrop-blur-md animate-fade-in" onClick={() => setPasseioSelecionado(null)} />
                    <div className="relative bg-carapita-green w-full max-w-5xl max-h-[90vh] overflow-y-auto z-[101] shadow-2xl flex flex-col md:flex-row animate-scale-up border border-white/10 rounded-[2rem]">
                        <button
                            className="absolute top-6 right-6 z-[102] w-12 h-12 bg-white/10 hover:bg-carapita-gold text-white rounded-full flex items-center justify-center transition-all backdrop-blur-md cursor-pointer"
                        ...
                        <div className="w-full md:w-1/2 p-12 md:p-20 flex flex-col justify-center bg-carapita-green">
                            <span className="text-carapita-gold uppercase tracking-widest text-xs font-medium block mb-6 border-b border-carapita-gold/30 pb-4">
                                {t('passeios_descobrir')}
                            </span>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4">
                                <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(passeioSelecionado.nome + " Portugal")}`}
                                ...
                                <button
                                    onClick={() => setPasseioSelecionado(null)}
                                    className="px-10 py-5 bg-white/5 hover:bg-white/10 border border-white/10 text-white text-[10px] uppercase tracking-mega rounded-xl transition-all font-bold cursor-pointer">
                                    {t('passeios_fechar')}
                                </button>
                            </div>
                        </div>
            ...
            <style jsx global>{`
                ...
                .pattern-dots { background-image:radial-gradient(circle,#fff 1px,transparent 1px); background-size:30px 30px; }
            `}</style>
        </main>
    );
}
